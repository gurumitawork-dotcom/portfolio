...
export default function Teaching() {
  return (
    <>
      <PageHero eyebrow="Teaching & Research" title="Building programs, not just publications" />
            ...
            <div className="px-5 sm:px-8 py-6 sm:py-7 sm:flex sm:items-start sm:justify-between sm:gap-10">
              <div className="min-w-0 max-w-2xl">
                ...
                <ul className="mt-4 space-y-2">
                  {CIMS_STORY.contributions.map((item) => (
                    <li key={item} className="text-sm text-slate-600 leading-relaxed pl-3 border-l-2 border-teal-600/30">
                      {item}
                    </li>
                  ))}
                </ul>
              ...
              <div className="mt-6 sm:mt-0 shrink-0 flex sm:flex-col gap-6 sm:gap-5 sm:text-right">
                {CIMS_STORY.figures.map((f) => (
                  <div key={f.value}>
                    <p className="font-serif text-3xl sm:text-4xl text-teal-700 leading-none">{f.value}</p>
        ...
        <div className="container-lg grid lg:grid-cols-2 gap-6">
          <Reveal className="glass-card relative overflow-hidden">
            ...
            {TEACHING_POSTS.map((post) => (
              ...
              >
                <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-teal-700 sm:pt-0.5">{post.years}</p>
          ...
          <Reveal delay={0.06} className="glass-card relative overflow-hidden">
            ...
            {FACULTY_MEETINGS.map((f) => (
              ...
              >
                <p className="text-sm font-semibold text-teal-700 sm:pt-0.5">{f.venue}</p>
                <div className="min-w-0">
        ...
        <div className="container-lg">
          <Reveal className="glass-card relative overflow-hidden">
            ...
            {GRAND_ROUNDS.map((group) => (
              ...
              >
                <p className="font-serif text-base text-teal-700 sm:pt-0.5">{group.year}</p>
